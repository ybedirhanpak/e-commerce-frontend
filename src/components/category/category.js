...
export default class Category extends Component {
  ...
  generateSection = index => {
    const subheaderPath = `/show/${this.props.mainCategory.path}/${
      this.props.sections[index].header.path
    }/`;
    return (
      <>
        <Link to={subheaderPath}>
          {" "}
          <li className="dropdown-header">
            {this.props.sections[index].header.name}
          </li>{" "}
        </Link>
  ...
  generateSubCategories = section => {
    ...
    for (var i = 0; i < section.subcategories.length; i++) {
      const subcategoryPath = `/show/${this.props.mainCategory.path}/${
        section.header.path
      }/${section.subcategories[i].path}`;
      resultList.push(
        <li key={i}>
          <Link
            to={{
